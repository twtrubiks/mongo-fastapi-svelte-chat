...
| Hook 事件 | 腳本 | 行為 |
|-----------|------|------|
| `PreToolUse(Write\|Edit)` | `pre-write-check.sh` | 攔截 .env / node_modules / .git 寫入（範本檔 `.env.example`/`.sample`/`.template` 放行，入版控且不含 secret）；掃描硬編碼 secrets（AWS Key、API Key、明文密碼）→ exit 2 攔截 |
