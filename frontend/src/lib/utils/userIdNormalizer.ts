 ...
 */
export function extractUserId(user: User | Record<string, unknown> | null): string | null {
  if (!user || typeof user !== 'object') {
    return null;
  }

  // 嘗試不同的 ID 字段格式
  const possibleIdFields = ['id', '_id', 'user_id', 'userId'] as const;
 ...
 * @returns 標準化的用戶對象
 */
export function normalizeUser(user: User | Record<string, unknown>): User {
  if (!user || typeof user !== 'object') {
 ...
 * @returns 標準化的用戶列表
 */
export function normalizeUserList(users: (User | Record<string, unknown>)[]): User[] {
