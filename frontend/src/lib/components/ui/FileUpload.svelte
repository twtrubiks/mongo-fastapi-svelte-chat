<script lang="ts">
  ...
  import type { BFFResponse } from '$lib/bff-types';
  import type { FileUploadData } from '$lib/bff-api-client';

  /** 單個檔案上傳結果 */
  interface UploadResult {
    file: File;
    result?: BFFResponse<FileUploadData>;
    error?: unknown;
  }
  ...
  interface Props {
    ...
    onUploaded?: (data: { files: File[]; results: UploadResult[] }) => void;
  ...
  let previewUrls = new SvelteMap<File, string>();
  ...
  export async function uploadToServer() {
    ...
    try {
      ...
      isUploading = false;
      uploadProgress = 0;
      const err = error as { detail?: string; error?: { message?: string }; message?: string };
      onError?.({ message: '上傳失敗: ' + (err.detail ?? err.error?.message ?? err.message ?? '未知錯誤') });
    }
  }
...
<style>
  ...
  /* 拖拽指示器 */
  .drag-indicator {
    @apply absolute inset-0 bg-primary/10 backdrop-blur-sm rounded-lg flex items-center justify-center z-10;
  }
