<script lang="ts">
  ...
  let messageContainer: HTMLDivElement = $state(null!);
  ...
  function handleScroll() {
    ...
    // 如果滾動到頂部且有更多訊息，自動載入
    // 使用 <= 5 而非 === 0，避免部分瀏覽器/裝置 scrollTop 為浮點數導致永遠不觸發
    if (messageContainer.scrollTop <= 5 && hasMore && !loading) {
      loadMoreMessages();
    }
  ...
  // 打字指示器出現時自動捲動
  $effect(() => {
    if (typingUsers.length > 0 && isAtBottom && messageContainer) {
      const gap = messageContainer.scrollHeight - messageContainer.scrollTop - messageContainer.clientHeight;
      if (gap > 1) {
        tick().then(() => {
          messageContainer.scrollTo({
            top: messageContainer.scrollHeight,
      ...
      <div class="typing-indicator">
        ...
        <span class="typing-text">
          ...
          {:else if typingUsers.length === 2}
            {typingUsers[0]?.username} 和 {typingUsers[1]?.username} 正在輸入...
          {:else}
            {typingUsers[0]?.username} 和其他 {typingUsers.length - 1} 人正在輸入...
          {/if}
        </span>
      </div>
    {/if}
...
<style>
  ...
  .typing-indicator {
    @apply flex items-center gap-2 px-4 py-2 text-sm text-base-content/60;
    min-height: 28px;
  }
  ...
  .typing-dots .dot {
    @apply w-1.5 h-1.5 rounded-full bg-base-content/40;
