<script lang="ts">
  ...
  async function performSearch() {
    ...
    try {
      const query: Parameters<typeof apiClient.messages.search>[1] = {
        page: currentPage,
        page_size: pageSize,
      };
      ...
      if (searchType !== 'all') {
        query.message_type = searchType as 'text' | 'image' | 'file' | 'system';
      }
  ...
  // 先 escapeHtml 再插入 <mark>，確保使用者訊息不會觸發 XSS
  function highlightKeyword(content: string, keyword: string): string {
    const escaped = escapeHtml(content);
    if (!keyword) return escaped;
    const pattern = escapeRegExp(escapeHtml(keyword));
...
{#if isOpen}
  <div class="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4" role="presentation" onclick={() => {
    onClose?.();
  }}>
    <div class="bg-base-100 border-2 border-base-300 rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden" role="dialog" aria-label="搜尋訊息" tabindex="-1" onclick={(e) => e.stopPropagation()} onkeydown={(e) => e.stopPropagation()}>
      <!-- 標題列 -->
      <div class="flex items-center justify-between p-4 border-b border-base-300 bg-base-200">
        ...
        <button class="btn btn-circle btn-ghost btn-sm" aria-label="關閉搜尋" onclick={() => {
          onClose?.();
        }}>
      ...
      <div class="p-4 border-b border-base-300 bg-base-50">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="form-control">
            <label class="label" for="search-keyword">
            ...
            {#if searchHistory.length > 0 && !searchKeyword}
              <div class="mt-2">
                ...
                <div class="flex flex-wrap gap-1">
                  {#each searchHistory as keyword (keyword)}
                    <button 
          ...
          <!-- 訊息類型 -->
          <div class="form-control">
            <label class="label" for="search-type">
              <span class="label-text font-medium text-base-content">訊息類型</span>
            </label>
            <select id="search-type" class="select select-bordered" bind:value={searchType}>
          ...
          <!-- 日期範圍 -->
          <div class="form-control">
            <label class="label" for="search-start-date">
          ...
          <div class="form-control">
            <label class="label" for="search-end-date">
            ...
            {#each searchResults as message (message.id)}
              ...
              >
                <div class="flex items-start gap-3">
                  <!-- 使用者頭像 -->
                  <Avatar
                    user={{ username: message.username ?? '', avatar: message.user?.avatar ?? message.avatar ?? '' }}
                    size="md"
                  />
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    {#if message.message_type === 'text'}
                      <p class="text-sm break-words">
                        {#if searchKeyword}
                          <!-- eslint-disable-next-line svelte/no-at-html-tags -- 搜尋關鍵字高亮需要插入 <mark> 標籤，內容已經過 escapeHtml 處理防止 XSS -->
                          {@html highlightKeyword(message.content, searchKeyword)}
                        {:else}
                          {message.content}
                        {/if}
                      </p>
