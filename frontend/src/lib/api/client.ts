...
interface BFFError extends Error {
  code: string;
  details?: unknown;
  response?: AxiosResponse<BFFResponse> | undefined;
}

// 創建 BFF 錯誤
function createBFFError(message: string, code: string, details?: unknown, response?: AxiosResponse | undefined): BFFError {
...
export class ApiClient {
  ...
  constructor(baseURL: string = '') {
    ...
    // FormData 請求攔截器：移除預設 Content-Type，讓瀏覽器自動帶 multipart/form-data + boundary
  ...
  // BFF 響應檢測
  private isBFFResponse(data: unknown): data is BFFResponse {
    if (typeof data !== 'object' || data === null) return false;
    const obj = data as Record<string, unknown>;
    if (typeof obj['success'] !== 'boolean') return false;
    if (!('data' in obj || 'error' in obj)) return false;
    // 補強：error 存在時驗證結構
    if (obj['error'] != null) {
