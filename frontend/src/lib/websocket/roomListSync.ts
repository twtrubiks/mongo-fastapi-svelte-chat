...
import type { WSRoomCreatedMessage, WSRoomDeletedMessage, WSRoomUpdatedMessage } from '$lib/types';

let isInitialized = false;
let eventHandlers: {
  handleRoomCreated?: (data: WSRoomCreatedMessage) => void;
  handleRoomDeleted?: (data: WSRoomDeletedMessage) => void;
  handleRoomUpdated?: (data: WSRoomUpdatedMessage) => void;
} = {};
...
export function setupRoomListSync() {
  ...
  try {
    ...
    // 監聽房間創建事件
    const handleRoomCreated = (data: WSRoomCreatedMessage) => {
      try {
        if (data && data.room) {
          // 將新房間添加到房間列表
    ...
    // 監聽房間刪除事件
    const handleRoomDeleted = (data: WSRoomDeletedMessage) => {
      try {
    ...
    // 監聽房間更新事件
    const handleRoomUpdated = (data: WSRoomUpdatedMessage) => {
      try {
        if (data && data.room) {
          // 即時更新房間資訊
