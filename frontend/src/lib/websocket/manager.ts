...
import type { HandlerContext, HandlerEvent } from './handlers';
...
export class WebSocketManager {
  ...
  private handleMessage(data: WebSocketMessage) {
    ...
    switch (data.type) {
      ...
      case 'user_joined':
        handleUserJoined(data.user || data.payload, data.timestamp, ctx);
        break;

      case 'user_left':
        handleUserLeft(data.user || data.payload, data.timestamp, ctx, data.removed);
        break;
      ...
      case 'notification_status_changed':
        handleNotificationStatusChanged(data.data || data.payload, ctx);
        break;

      case 'room_created':
        handleRoomCreated(data.data || data.payload || data, ctx);
        break;

      case 'room_deleted':
        handleRoomDeleted(data.data || data.payload || data, ctx);
        break;

      case 'room_updated':
        handleRoomUpdated(data.data || data.payload || data, ctx);
        break;
      ...
      case 'error':
        handleError(data.payload, ctx);
        break;

      default:
        // 未處理的事件類型，透過通用 emit 轉發
  ...
  // 事件監聽
  on(event: HandlerEvent, callback: (data: any) => void) {
  ...
  off(event: HandlerEvent, callback: (data: any) => void) {
    if (this.eventListeners[event]) {
