...
import type { Notification, WSNotificationMessage, WSRoomCreatedMessage, NotificationStatusData } from '$lib/types';
...
export function dispatchNotification(data: WSNotificationMessage, ctx: HandlerContext) {
  ...
  if (notificationData['type'] === 'room_created') {
    // 向下相容：房間創建事件可能包裝在 notification 內
    ctx.emit('room_created', notificationData as unknown as WSRoomCreatedMessage);
  } else {
    // 轉換通知資料格式
...
// 處理通知狀態變更
export function handleNotificationStatusChanged(data: NotificationStatusData, ctx: HandlerContext) {
  try {
    const { read_type, id, status } = data;

    if (status === 'read') {
      if (read_type === 'notification') {
        notificationStore.markAsRead(id);
      } else if (read_type === 'room') {
      ...
      } else if (read_type === 'all') {
        notificationStore.markAllAsRead();
      }
    }

    ctx.emit('read_status_updated', data);
