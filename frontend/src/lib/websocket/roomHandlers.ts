...
// 處理用戶離開
export function handleUserLeft(user: User | string, eventTimestamp: string | undefined, ctx: HandlerContext, removed?: boolean) {
...
// 處理全局在線狀態變更
export function handleUserStatusChanged(data: any) {
...
// 處理房間用戶列表
export function handleRoomUsers(data: any) {
...
// 處理房間創建事件
export function handleRoomCreated(data: any, ctx: HandlerContext) {
  try {
    // 檢查是否有新房間資料
    if (!data || !data.room) {
...
export function handleRoomDeleted(data: any, ctx: HandlerContext) {
  try {
    ...
    console.error('[WebSocket] 處理房間刪除事件失敗:', error);
  }
}

// 處理房間更新事件（store 更新由 roomListSync 統一處理）
export function handleRoomUpdated(data: any, ctx: HandlerContext) {
  try {
    if (!data || !data.room) {
      return;
    }
    ...
    console.error('[WebSocket] 處理房間更新事件失敗:', error);
  }
}
