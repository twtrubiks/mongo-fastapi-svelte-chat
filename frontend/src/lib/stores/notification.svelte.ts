...
// 處理通知數據的規範化函數
function processNotification(n: Partial<Notification>): Notification | null {
  ...
  return {
    ...
    ...(n.metadata != null && { metadata: n.metadata }),
    ...(n.room_id != null && { room_id: n.room_id }),
    ...(n.sender_id != null && { sender_id: n.sender_id }),
  };
}
...
function getStoredNotifications(): Notification[] {
  ...
  try {
    ...
    if (stored) {
      const parsed = JSON.parse(stored);
      return (parsed as Partial<Notification>[]).map((n) => processNotification(n)).filter((n): n is Notification => n !== null);
    }
  } catch (error) {
