...
// 以 refresh_token 值為 key 去重，避免跨使用者認證混線
const refreshingMap = new Map<string, Promise<string | null>>();

/**
 * 嘗試用 refresh_token 換發新的 access_token
 *
 * 內建 promise 去重：同一使用者的並行呼叫只會發出一次 refresh 請求。
 *
...
async function tryRefreshToken(cookies: Cookies): Promise<string | null> {
	...
	const existing = refreshingMap.get(refreshToken);
	if (existing) return existing;

	const promise = doRefresh(cookies, refreshToken);
	refreshingMap.set(refreshToken, promise);
	try {
		return await promise;
	} finally {
		refreshingMap.delete(refreshToken);
	}
}

async function doRefresh(cookies: Cookies, refreshToken: string): Promise<string | null> {
