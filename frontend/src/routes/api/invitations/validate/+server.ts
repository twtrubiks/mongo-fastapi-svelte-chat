...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (error.code) {
      ...
      if (error.code === 400 || error.code === 404 || error.code === 422 || error.code === 500) {
    ...
    if (error.response) {
      ...
      if (status === 400 || status === 404 || status === 422 || status === 500) {
        return json(createBFFError('INVALID_INVITE', data.detail ?? data.error?.message ?? '邀請碼無效或已過期'), { status: 400 });
      }
      
      return json(createBFFError('BACKEND_ERROR', data.detail ?? data.error?.message ?? '後端處理錯誤'), { status: status });
    }
