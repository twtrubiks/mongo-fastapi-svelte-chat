...
from app.models.room import RoomCreate, RoomJoinRequest, RoomResponse, RoomUpdate
...
async def update_room(
    ...
    try:
        ...
        updated_room = await room_service.update_room(
            room_id, current_user_id, room_update
        )
...
async def delete_room(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="聊天室不存在或無權限刪除"
            )
...
async def leave_room(
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="無法離開聊天室"
            )
