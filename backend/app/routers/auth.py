...
from app.models.user import UserCreate, UserUpdate, UserResponse, Token, AuthResponse
...
async def register(user: UserCreate, service: UserService = UserServiceDep):
    """
    使用者註冊
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), service: UserService = UserServiceDep):
    """
    使用者登入
    ...
    try:
        # 使用 UserService 進行認證
        auth_result = await service.authenticate_user(form_data.username, form_data.password)
