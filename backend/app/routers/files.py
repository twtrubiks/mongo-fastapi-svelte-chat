...
from app.utils.file_upload import get_mime_type, upload_file
...
def _validate_filename(filename: str) -> str:
    """驗證檔案名稱，防止路徑遍歷攻擊"""
    safe = PathLib(filename).name
    if safe != filename:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="無效的檔案名稱"
        )
    return safe


def _validate_path_within(file_path: PathLib, base_dir: PathLib) -> PathLib:
    """驗證解析後的路徑在允許的基礎目錄內，防止 symlink 逃逸"""
    resolved = file_path.resolve()
    if not resolved.is_relative_to(base_dir.resolve()):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="無效的檔案路徑"
        )
    return resolved
...
async def get_thumbnail(
    ...
    # 構建縮圖路徑
    file_stem = PathLib(safe_filename).stem
    file_ext = PathLib(safe_filename).suffix
    thumbnail_filename = f"{file_stem}_{size}{file_ext}"
    thumbnail_dir = PathLib("processed") / "thumbnails"
    thumbnail_path = _validate_path_within(
        thumbnail_dir / thumbnail_filename, thumbnail_dir
    )

    # 檢查縮圖是否存在
    if not thumbnail_path.exists():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="縮圖不存在")
...
async def download_file(
    ...
    # 構建檔案路徑並驗證在 UPLOAD_DIR 範圍內
    upload_base = PathLib(settings.UPLOAD_DIR)
    file_path = _validate_path_within(
