...
class ServiceDescriptor:
    ...
    def __init__(
        ...
        if sum(bool(x) for x in [implementation_type, factory, instance]) != 1:
            raise ValueError(
                "必須提供 implementation_type、factory 或 instance 中的一個"
            )
...
class DIContainer:
    ...
    async def get(self, service_type: type[T], scope_id: str | None = None) -> T:
        """
        獲取服務實例
    ...
    async def _get_scoped(
        self, descriptor: ServiceDescriptor, scope_id: str | None
    ) -> Any:
        """獲取作用域服務"""
        if scope_id is None:
            scope_id = "default"
        ...
        logger.debug(
            f"Created scoped instance: {service_type.__name__} (scope: {scope_id})"
        )
        return instance
