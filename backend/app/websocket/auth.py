...
async def websocket_auth_middleware(websocket: WebSocket, room_id: str) -> Optional[Dict[str, Any]]:
    """
    WebSocket 認證中介軟體
    ...
    try:
        ...
        if not await verify_room_membership(user_info["id"], room_id):
            logger.warning(f"User {user_info['id']} has no permission to join room {room_id}")
            return None
