...
from app.models.message import MessageCreate, MessageType, MessageStatus
...
async def handle_websocket_connection(websocket: WebSocket, room_id: str):
    ...
    try:
        ...
        # 發送歡迎訊息
        await connection_manager.send_personal_message(user_id, room_id, {
            "type": "welcome",
        ...
        async with container.scope(f"websocket-{user_id}-{room_id}") as scope_id:
            ...
            while True:
                try:
                    ...
                    # 處理不同類型的訊息
                    await handle_message(user_id, room_id, message_data, user_info, scope_id)
...
async def handle_message(user_id: str, room_id: str, message_data: dict, user_info: dict, scope_id: str = None):
    """
    處理收到的訊息
    ...
    if message_type == "chat_message" or message_type == "message":
        ...
        await handle_notification_read(user_id, room_id, message_data, user_info, scope_id)
    else:
...
async def handle_chat_message(user_id: str, room_id: str, message_data: dict, user_info: dict, scope_id: str = None):
    """
    處理聊天訊息
    ...
    try:
        ...
        await connection_manager.broadcast_message(room_id, message_payload, None)  # 不排除任何用戶，確保發送者也能看到自己的訊息
        ...
        try:
            ...
            logger.info(f"Notifications sent to {len([m for m in room_members if m.id != user_id])} members")
        except Exception as e:
...
async def handle_typing_indicator(user_id: str, room_id: str, message_data: dict, user_info: dict):
    """
    處理打字指示器
...
async def handle_notification_read(user_id: str, room_id: str, message_data: dict, user_info: dict, scope_id: str = None):
    """
    處理通知已讀狀態變更
    ...
    try:
        ...
        read_type = message_data.get("read_type", "notification")  # 'notification' | 'room'
        ...
        if read_type == "notification" and notification_id:
            ...
            # 標記房間相關通知為已讀
            try:
                logger.info(f"[WebSocket] Processing room notification read request: user={user_id}, room={target_room_id}")
                logger.info(f"[WebSocket] Message data: {message_data}")
                # 標記該房間所有未讀通知為已讀
                count = await notification_service.mark_room_notifications_as_read(user_id, target_room_id)
                success = count >= 0  # 即使沒有未讀通知也算成功
                logger.info(f"Marked {count} room notifications as read for user {user_id} in room {target_room_id}")
            except Exception as e:
        ...
        if success:
            ...
            # 發送到用戶的所有活動連接（跨房間）
            await connection_manager.send_notification(user_id, status_change_data["data"])
            
            logger.info(f"Broadcasted read status change to all connections for user {user_id}")
