...
class ConnectionManager:
    ...
    def is_user_in_room(self, user_id: str, room_id: str) -> bool:
        ...
        Returns:
            bool: 是否在房間中
        """
        return room_id in self.room_users and user_id in self.room_users[room_id]
        ...
        if user_id in self.active_connections:
            ...
            if room_ids:
                ...
                try:
                    ...
                    await self.send_personal_message(
                        user_id, room_id, notification_message
                    )
                    logger.debug(
                        f"Notification sent to {user_id} via room {room_id}: {notification_data.get('title')}"
                    )
                except Exception as e:
                    logger.error(
                        f"Error sending notification to {user_id} in room {room_id}: {e}"
                    )
                    ...
                    for room_id in room_ids[1:]:
                        try:
                            ...
                            logger.debug(
                                f"Notification sent to {user_id} via fallback room {room_id}"
                            )
                            break
                        except Exception as e:
                            logger.error(
                                f"Error sending notification to {user_id} in fallback room {room_id}: {e}"
                            )
                            continue
            else:
                logger.debug(
                    f"User {user_id} has no active connections for notification"
                )
        else:
            # 將警告降級為 debug，因為用戶離線是正常情況
            logger.debug(
                f"User {user_id} not found in active connections for notification (user is offline)"
            )
