...
class RoomRepository(BaseRepository[RoomInDB]):
    ...
    async def get_by_id(self, room_id: str) -> RoomInDB | None:
        """
        根據 ID 獲取房間
    ...
    async def create(self, room: RoomInDB) -> RoomInDB | None:
        """
        創建新房間
    ...
    async def update(self, room_id: str, room_update: RoomUpdate) -> RoomInDB | None:
        """
        更新房間
    ...
    async def search_rooms(
        self, keyword: str, skip: int = 0, limit: int = 20, public_only: bool = True
    ) -> list[RoomInDB]:
        """
        搜尋房間
    ...
    async def add_member(self, room_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            # 添加成員
            success = await self.update_one(
                {"_id": ObjectId(room_id)}, {"$push": {"members": user_id}}
            )
    ...
    async def remove_member(self, room_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            # 移除成員
            success = await self.update_one(
                {"_id": ObjectId(room_id)}, {"$pull": {"members": user_id}}
            )
    ...
    async def transfer_ownership(self, room_id: str, new_owner_id: str) -> bool:
        ...
        try:
            ...
            if new_owner_id not in room.members:
                logger.warning(
                    f"New owner {new_owner_id} is not a member of room {room_id}"
                )
                return False

            # 轉移擁有權
            success = await self.update_one(
                {"_id": ObjectId(room_id)}, {"$set": {"owner_id": new_owner_id}}
            )
    ...
    async def get_room_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            avg_members_pipeline = [
                ...
                {"$group": {"_id": None, "avg_members": {"$avg": "$member_count"}}},
            ]
    ...
    async def create_indexes(self):
        ...
        try:
            ...
            # 公開狀態和創建時間複合索引
            await self.create_index([("is_public", 1), ("created_at", -1)])
