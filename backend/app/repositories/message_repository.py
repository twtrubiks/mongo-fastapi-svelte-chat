...
class MessageRepository(BaseRepository[MessageInDB]):
    ...
    async def get_by_id(self, message_id: str) -> MessageInDB | None:
        """
        根據 ID 獲取訊息
    ...
    async def get_message_stats(self, room_id: str) -> MessageStats:
        ...
        try:
            ...
            messages_today = await self.count_documents(
                {**base_query, "created_at": {"$gte": today_start}}
            )

            # 本週訊息數
            week_start = today_start - timedelta(days=today_start.weekday())
            messages_this_week = await self.count_documents(
                {**base_query, "created_at": {"$gte": week_start}}
            )

            # 本月訊息數
            month_start = today_start.replace(day=1)
            messages_this_month = await self.count_documents(
                {**base_query, "created_at": {"$gte": month_start}}
            )
            ...
            message_types = {
                item["_id"]: item["count"] for item in message_types_result
            }
    ...
    async def get_user_message_stats(self, user_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            total_messages = await self.count_documents(
                {"user_id": user_id, "status": {"$ne": MessageStatus.DELETED}}
            )
            ...
            return {"total_messages": 0, "today_messages": 0, "week_messages": 0}

    async def get_messages_by_user(
        self, room_id: str, user_id: str, page: int = 1, page_size: int = 20
    ) -> list[MessageInDB]:
        """
        獲取特定使用者在房間的訊息
        ...
        try:
            ...
            return await self.get_room_messages(
                room_id=room_id, skip=skip, limit=page_size, user_id=user_id
            )
        except Exception as e:
    ...
    async def delete_messages_by_user(self, room_id: str, user_id: str) -> int:
        ...
        try:
            ...
            logger.info(
                f"Deleted {modified_count} messages by user {user_id} in room {room_id}"
            )
            return modified_count
        except Exception as e:
    ...
    async def create_indexes(self):
        ...
        try:
            # 房間和創建時間複合索引
            await self.create_index([("room_id", 1), ("created_at", -1)])

            # 房間、類型和創建時間複合索引
            await self.create_index(
                [("room_id", 1), ("message_type", 1), ("created_at", -1)]
            )

            # 使用者和房間複合索引
            await self.create_index(
                [("user_id", 1), ("room_id", 1), ("created_at", -1)]
            )
    ...
    async def cleanup_old_messages(self, days: int = 30) -> int:
        ...
        try:
            ...
            deleted_count = await self.delete_many(
                {"created_at": {"$lt": cutoff_date}, "status": MessageStatus.DELETED}
            )
