...
class NotificationRepository(BaseRepository[NotificationInDB]):
    ...
    async def get_by_id(self, notification_id: str) -> NotificationInDB | None:
        """
        根據 ID 獲取通知
    ...
    async def create(self, notification: NotificationInDB) -> NotificationInDB | None:
        """
        創建通知
    ...
    async def get_by_user(
        ...
        try:
            ...
            notifications = await self.find_many(
    ...
    async def get_unread_count(self, user_id: str) -> int:
        ...
        """
        try:
            count = await self.count_documents({"user_id": user_id, "is_read": False})
            return count
        except Exception as e:
    ...
    async def get_stats(self, user_id: str) -> NotificationStats:
        ...
        try:
            ...
            dismissed_count = await self.count_documents(
                {"user_id": user_id, "status": NotificationStatus.DISMISSED}
            )
            ...
            recent_notifications = await self.find_many(
                {"user_id": user_id}, skip=0, limit=5, sort=[("created_at", -1)]
            )
