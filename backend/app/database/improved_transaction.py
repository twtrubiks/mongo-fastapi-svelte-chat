...
class ImprovedTransactionManager:
    ...
    def __init__(self):
        ...
        self._current_context: ContextVar[TransactionContext | None] = ContextVar(
    ...
    def get_current_context(self) -> TransactionContext | None:
        """獲取當前事務上下文"""
    ...
    @asynccontextmanager
    async def transaction(self, isolation_level: str | None = None):
        """
        改進的事務上下文管理器
    ...
    async def _handle_nested_transaction(self, context: TransactionContext):
        ...
        async with self._lock:
            ...
            logger.debug(
                f"Entering nested transaction level {context.nested_level} for {context.transaction_id}"
            )

        try:
            yield context.session
            logger.debug(
                f"Nested transaction level {context.nested_level} completed for {context.transaction_id}"
            )
        finally:
            async with self._lock:
                context.nested_level -= 1
    ...
    @asynccontextmanager
    async def _handle_new_transaction(self, isolation_level: str | None = None):
        """處理新事務"""
            ...
            async with client.start_session() as session:
                try:
                    ...
                    if isolation_level:
                        ...
                        logger.info(
                            f"Transaction isolation level requested: {isolation_level}"
                        )
                    ...
                    try:
                        ...
                        logger.info(
                            f"Transaction {transaction_id} committed successfully (operations: {context.operations_count})"
                        )
    ...
    async def get_transaction_stats(self) -> dict[str, Any]:
        ...
        return {"active_transactions": active_count, "contexts": contexts_info}
...
def improved_transactional(
    retry_count: int = 0, retry_delay: float = 0.1, timeout: float | None = None
):
    """
    改進的事務裝飾器
            ...
            for attempt in range(retry_count + 1):
                try:
                    ...
                    if attempt < retry_count:
                        logger.warning(
                            f"Transaction attempt {attempt + 1} failed, retrying in {retry_delay}s: {e}"
                        )
...
class ImprovedTransactionMixin:
    ...
    async def _get_transaction_info(self) -> dict[str, Any] | None:
        """獲取當前事務資訊（用於調試）"""
