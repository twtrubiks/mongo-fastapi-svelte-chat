...
class RoomService:
    ...
    async def _broadcast_room_created(
        self, connection_manager, room_response: RoomResponse, creator_id: str
    ):
        """
        ...
        try:
            ...
            logger.info(
                f"廣播新房間 {room_response.id} 給 {len(all_active_users)} 個用戶"
            )
        ...
        try:
            ...
            if room_data.password:
                ...
                password_hash = bcrypt.hashpw(
                    room_data.password.encode("utf-8"), bcrypt.gensalt()
                ).decode("utf-8")
            ...
            if created_room:
                ...
                try:
                    ...
                    # 廣播給所有連接的用戶（除了創建者）
                    await self._broadcast_room_created(
                        connection_manager, room_response, owner_id
                    )
                except Exception as e:
            ...
            raise Exception("創建房間失敗") from e

    async def get_room_by_id(self, room_id: str) -> RoomResponse | None:
        """
        根據 ID 獲取房間
    ...
    async def update_room(
        ...
        if update_data.max_members is not None:
            ...
            if update_data.max_members < current_member_count:
                raise ValueError(
                    f"最大成員數不能小於當前成員數 ({current_member_count})"
                )
    ...
    async def transfer_ownership(
        self, room_id: str, current_owner_id: str, new_owner_id: str
    ) -> dict[str, Any]:
        """
        轉移房間擁有權（使用事務確保資料一致性）
        ...
        if success:
            logger.info(
                f"Room ownership transferred: {room_id} from {current_owner_id} to {new_owner_id}"
            )
