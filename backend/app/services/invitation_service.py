...
class InvitationService:
    ...
    async def create_invitation(
        ...
        # 儲存到資料庫
        result = await self.invitations_collection.insert_one(
            invitation.model_dump(exclude={"id"})
        )
        ...
        return InvitationResponse(**invitation.model_dump(), invite_link=invite_link)

    async def validate_invitation(self, invite_code: str) -> RoomInvitation | None:
        """
        驗證邀請碼
    ...
    async def use_invitation(self, invite_code: str) -> bool:
        ...
        result = await self.invitations_collection.update_one(
            {"invite_code": invite_code}, {"$inc": {"uses_count": 1}}
        )
    ...
    async def get_room_invitations(
        ...
        async for doc in cursor:
            ...
            invite_link = (
                f"http://localhost:5173/app/rooms/join?code={invitation.invite_code}"
            )
            invitations.append(
    ...
    async def create_join_request(
        ...
        # 儲存到資料庫
        result = await self.join_requests_collection.insert_one(
            join_request.model_dump(exclude={"id"})
        )
    ...
    async def review_join_request(
        ...
        request_doc = await self.join_requests_collection.find_one(
            {"_id": request_id, "status": JoinRequestStatus.PENDING.value}
        )
        ...
        if review.status == JoinRequestStatus.APPROVED:
            ...
            await self.room_repo.update_member_role(
                join_request.room_id, join_request.requester_id, "member"
            )
