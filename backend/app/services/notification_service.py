...
class NotificationService:
    ...
    async def create_notification(
        ...
        """
        # intentional fail-open: 通知為非核心功能，建立失敗不應中斷主流程（如發訊息）
        try:
    ...
    async def get_user_notifications(
        ...
        """
        # intentional fail-open: 通知列表查詢失敗時回傳空列表，避免影響整體 UI 渲染
        try:
            ...
            total, unread_count = await asyncio.gather(
                self.notification_repo.count_documents(total_query),
    ...
    async def clear_user_notifications(self, user_id: str) -> int:
        ...
        Returns:
            int: 刪除的通知數量
        """
        # intentional fail-open: 清空通知失敗回傳 0，不中斷使用者操作
        try:
    ...
    async def get_notification_stats(self, user_id: str) -> NotificationStats:
        ...
        """
        # intentional fail-open: 統計查詢失敗回傳空統計，不影響主畫面載入
        try:
