...
class UserService:
    ...
    async def create_user(self, user_data: UserCreate) -> UserResponse:
        ...
        try:
            ...
            raise ValueError("使用者名稱已存在") from e
        except EmailAlreadyExistsError as e:
            logger.warning(f"Email already exists: {e}")
            raise ValueError("信箱已被使用") from e
        except Exception as e:
            logger.error(f"Error creating user: {e}")
            raise Exception("創建使用者失敗") from e
    ...
    async def get_current_user_from_token(self, token: str) -> dict[str, Any] | None:
        """
        從 token 獲取當前使用者
    ...
    async def get_online_users(self, room_id: str | None = None) -> list[UserResponse]:
        """
        獲取在線使用者列表
