...
class MessageBase(BaseModel):
    ...
    reply_to: Optional[str] = Field(None, description="回覆的訊息 ID")
    metadata: Optional[Dict[str, Any]] = Field(None, description="訊息元數據")
...
class MessageUpdate(BaseModel):
    """更新訊息模型"""
    content: Optional[str] = Field(None, min_length=1, max_length=10000, description="訊息內容")
    status: Optional[MessageStatus] = Field(None, description="訊息狀態")
    metadata: Optional[Dict[str, Any]] = Field(None, description="訊息元數據")


class MessageInDB(MessageBase):
    """資料庫中的訊息模型"""
    id: Optional[str] = Field(None, alias="_id", description="訊息 ID")
    room_id: str = Field(..., description="房間 ID")
    ...
    updated_at: datetime = Field(default_factory=lambda: datetime.now(UTC), description="更新時間")
...
class MessageSearchQuery(BaseModel):
    """訊息搜尋查詢模型"""
    keyword: Optional[str] = Field(None, min_length=1, max_length=100, description="搜尋關鍵字")
    message_type: Optional[MessageType] = Field(None, description="訊息類型")
    user_id: Optional[str] = Field(None, description="發送者 ID")
...
class MessageWithReply(MessageResponse):
    """包含回覆資訊的訊息模型"""
    reply_to_message: Optional[ReplyToMessage] = Field(None, description="被回覆的訊息資訊")


class MessageBatch(BaseModel):
    """批次訊息模型"""
    message_ids: List[str] = Field(..., min_length=1, max_length=100, description="訊息 ID 列表")
    action: str = Field(..., description="操作類型")
    status: Optional[MessageStatus] = Field(None, description="目標狀態")
...
class MessageReaction(BaseModel):
    ...
    created_at: datetime = Field(default_factory=lambda: datetime.now(UTC), description="反應時間")
...
class MessageWithReactions(MessageResponse):
    """包含反應的訊息模型"""
    reactions: List[MessageReaction] = Field(default_factory=list, description="訊息反應列表")
    reaction_counts: Dict[str, int] = Field(default_factory=dict, description="反應統計")