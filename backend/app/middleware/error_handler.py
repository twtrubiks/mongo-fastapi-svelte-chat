...
class GlobalErrorHandler(BaseHTTPMiddleware):
    ...
    def __init__(self, app, debug: bool = False):
        ...
        self.error_mappings = {
            ...
            # Redis 錯誤
            RedisConnectionError: (status.HTTP_503_SERVICE_UNAVAILABLE, "CACHE_CONNECTION_ERROR"),
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        try:
            ...
            logger.debug(f"請求完成: {request.method} {request.url} - {response.status_code} ({duration:.3f}s)")
    ...
    async def handle_error(self, request: Request, exc: Exception) -> JSONResponse:
        ...
        try:
            ...
            if self.debug and not isinstance(exc, (HTTPException, BusinessLogicError, ValidationError)):
    ...
    def _create_error_detail(self, exc: Exception) -> ErrorDetail:
        ...
        # 處理自定義業務錯誤
        if isinstance(exc, (BusinessLogicError, ValidationError, NotFoundError, 
    ...
    async def _log_error(self, request: Request, exc: Exception, 
                        error_detail: ErrorDetail, status_code: int):
        """記錄錯誤詳情"""
        error_info = {
            "method": request.method,
            "url": str(request.url),
