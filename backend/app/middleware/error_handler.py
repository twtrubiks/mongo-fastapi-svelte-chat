...
class GlobalErrorHandler(BaseHTTPMiddleware):
    ...
    async def handle_error(self, request: Request, exc: Exception) -> JSONResponse:
        ...
        try:
            ...
            # debug 模式下將 traceback 記錄到日誌（不洩漏到 response）
            if self.debug and not isinstance(exc, HTTPException):
        ...
        except Exception as handler_error:  # intentional catch-all: 最後備援
            # 錯誤處理器本身出錯的最後備援
    ...
    async def _log_error(
        ...
        # 根據錯誤嚴重程度選擇日誌級別
        if status_code >= 500:
            logger.error(f"伺服器錯誤: {error_info}", exc_info=exc)
