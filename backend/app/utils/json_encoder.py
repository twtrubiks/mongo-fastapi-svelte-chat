...
def clean_data_for_json(data: Any) -> Any:
    ...
    if isinstance(data, dict):
        ...
        if data.tzinfo is None:
            # 如果沒有時區資訊，假設為 UTC
            return data.replace(tzinfo=UTC).isoformat().replace("+00:00", "Z")
        else:
            # 將 +00:00 格式轉換為 Z 格式，確保前端兼容性
...
def prepare_websocket_message(
    message_type: str, payload: Any = None, **extra_fields
) -> dict:
    """
    準備 WebSocket 訊息格式
    ...
    Returns:
        格式化的訊息字典
    """
    message = {"type": message_type, "timestamp": datetime.now(UTC).isoformat()}
