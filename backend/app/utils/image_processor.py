...
class ImageProcessor:
    ...
    def open_image(self, image_path: str) -> Image.Image:
        ...
        try:
            ...
            raise ImageProcessorError(f"無法打開圖片: {str(e)}")
                            ...
                            for tag, value in exif.items():
                                try:
                                    ...
                                    if isinstance(value, (int, float, str)):
                                        ...
                                        # 安全地處理字節數據
                                        try:
                                            exif_dict[decoded] = value.decode('utf-8', errors='ignore')[:100]  # 限制長度
                                        except:
                                            pass  # 跳過無法解碼的字節
                                        ...
                                        try:
                                            ...
                                            for item in value:
                                                if isinstance(item, (int, float, str)):
                                                    ...
                                                    safe_list.append(str(item)[:50])  # 轉字符串並限制長度
                                        ...
                                        try:
                                            str_value = str(value)[:100]  # 限制字符串長度
    ...
    def resize_image(
        ...
        try:
            ...
            if return_path:
                ...
                result_image.save(output_path, format=result_image.format or 'JPEG', quality=85, optimize=True)
            ...
            raise ImageProcessorError(f"調整圖片大小失敗: {str(e)}")
    ...
    def create_thumbnail(
        ...
        try:
            ...
            thumbnail_filename = f"{original_path.stem}_{size_name}{original_path.suffix}"
            thumbnail_path = self.thumbnail_dir / thumbnail_filename
            
            # 儲存縮圖
            thumbnail.save(thumbnail_path, format=image.format, quality=85, optimize=True)
            ...
            raise ImageProcessorError(f"創建縮圖失敗: {str(e)}")
    ...
    def compress_image(
        ...
        try:
            ...
            raise ImageProcessorError(f"壓縮圖片失敗: {str(e)}")
    ...
    def crop_image(
        ...
        try:
            ...
            # 儲存裁剪圖片
            cropped_image.save(cropped_path, format=image.format, quality=85, optimize=True)
            ...
            raise ImageProcessorError(f"裁剪圖片失敗: {str(e)}")
    ...
    def rotate_image(self, image_path: str, angle: float) -> str:
        ...
        try:
            ...
            # 儲存旋轉圖片
            rotated_image.save(rotated_path, format=image.format, quality=85, optimize=True)
            ...
            raise ImageProcessorError(f"旋轉圖片失敗: {str(e)}")
    ...
    def apply_filter(self, image_path: str, filter_type: str) -> str:
        ...
        try:
            ...
            # 生成濾鏡檔案名
            original_path = Path(image_path)
            filtered_filename = f"{original_path.stem}_{filter_type}{original_path.suffix}"
            filtered_path = self.output_dir / filtered_filename
            
            # 儲存濾鏡圖片
            filtered_image.save(filtered_path, format=image.format, quality=85, optimize=True)
            ...
            raise ImageProcessorError(f"應用濾鏡失敗: {str(e)}")
    ...
    def convert_format(self, image_path: str, target_format: str) -> str:
        ...
        try:
            ...
            raise ImageProcessorError(f"轉換圖片格式失敗: {str(e)}")
    ...
    def image_to_base64(self, image_path: str) -> str:
        ...
        try:
            ...
            raise ImageProcessorError(f"轉換圖片為 Base64 失敗: {str(e)}")
    ...
    def base64_to_image(self, base64_string: str, output_path: str) -> str:
        ...
        try:
            ...
            raise ImageProcessorError(f"轉換 Base64 為圖片失敗: {str(e)}")
    ...
    def create_watermark(
        ...
        try:
            ...
            # 生成浮水印檔案名
            original_path = Path(image_path)
            watermarked_filename = f"{original_path.stem}_watermarked{original_path.suffix}"
            ...
            raise ImageProcessorError(f"添加浮水印失敗: {str(e)}")
...
# 向後兼容的模組層級函數
def create_single_thumbnail(image_path: str, size: Tuple[int, int], output_path: Optional[str] = None) -> str:
    """
    創建單個縮圖的模組層級函數（向後兼容）
