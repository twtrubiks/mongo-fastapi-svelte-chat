...
class DateTimeUtils:
    ...
    def format_datetime(
        ...
        try:
            ...
            format_str = DateTimeUtils.DATE_FORMATS.get(
                format_name, "%Y-%m-%d %H:%M:%S"
            )
    ...
    def parse_datetime(
        ...
        try:
            ...
            raise DateTimeError(f"無法解析日期時間字符串: {date_string}") from e
    ...
    def convert_timezone(dt: datetime, target_tz: str) -> datetime:
        ...
        try:
            ...
            raise DateTimeError(f"無法轉換時區: {target_tz}") from e
    ...
    @staticmethod
    def get_age(birth_date: datetime, reference_date: datetime | None = None) -> int:
        """
        計算年齡
    ...
    @staticmethod
    def from_unix_timestamp(timestamp: int | float, tz: str | None = None) -> datetime:
        """
        從 Unix 時間戳創建日期時間物件
...
def format_datetime(
    dt: datetime, format_name: str = "log", format_str: str | None = None
) -> str:
    """格式化日期時間"""
    if format_str:
        return dt.strftime(format_str)
...
def parse_datetime(date_string: str, format_str: str | None = None) -> datetime:
    """解析日期時間字符串"""
    if format_str:
        return datetime.strptime(date_string, format_str)
...
def is_business_day(dt: datetime, holidays: list | None = None) -> bool:
    """檢查是否為工作日"""
    # 先檢查是否為週末
