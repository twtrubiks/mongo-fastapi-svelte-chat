...
class FileUploadManager:
    ...
    def check_duplicate_file(self, file_hash: str) -> str | None:
        """
        檢查重複檔案
...
async def upload_multiple_files(
    files: list[UploadFile], allowed_types: list[str] | None = None, max_files: int = 10
) -> list[dict[str, Any]]:
    """
    上傳多個檔案的便捷函數
            ...
            for file in files:
                ...
                if file_type not in allowed_types:
                    raise FileUploadError(
                        f"不允許的檔案類型: {file_type} ({file.filename})"
                    )
