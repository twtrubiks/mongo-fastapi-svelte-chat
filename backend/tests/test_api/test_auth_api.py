...
class TestAuthAPI:
    ...
    @pytest.mark.asyncio
    async def test_refresh_token_success(self, async_client, isolated_db_manager):
        """測試成功刷新 token"""
        # 註冊並登入
        test_user = {
    ...
    @pytest.mark.asyncio
    async def test_refresh_token_invalid(self, async_client, isolated_db_manager):
    ...
    @pytest.mark.asyncio
    async def test_refreshed_token_works(self, async_client, isolated_db_manager):
        """測試刷新後的 access token 可正常使用"""
        # 註冊並登入
        test_user = {
            "username": "refreshworksuser",
            "email": "refreshworks@example.com",
            "password": "testpass123",
            "full_name": "Refresh Works User",
        }
        await async_client.post("/api/auth/register", json=test_user)
