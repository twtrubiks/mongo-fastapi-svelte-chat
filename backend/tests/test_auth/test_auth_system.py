...
class TestAuthSystem:
    ...
    async def test_get_current_user_dependency(self):
        """測試獲取當前用戶的依賴（透過 fetch_user_with_cache）"""
        ...
        # 模擬 cache miss → 查 MongoDB（透過 UserRepository）→ 寫入快取
        with (
            patch(
            ...
            patch("app.utils.user_cache.get_database", new_callable=AsyncMock),
            patch(
                "app.utils.user_cache.UserRepository",
                return_value=mock_repo,
            ),
            patch("app.utils.user_cache.set_cached_user", new_callable=AsyncMock),
        ):
    ...
    async def test_refresh_access_token_success(self):
        """測試使用 refresh token 成功換發新 token"""
        user_data = {"sub": "testuser", "user_id": "507f1f77bcf86cd799439011"}
        ...
        # 驗證新的 access token 可以解碼
        payload = decode_access_token(result["access_token"])
        assert payload is not None
        assert payload["sub"] == "testuser"

        # 驗證新的 refresh token 可以解碼
