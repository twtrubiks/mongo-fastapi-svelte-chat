...
from app.repositories.user_repository import UserAlreadyExistsError, EmailAlreadyExistsError
...
class TestUserService:
    ...
    @pytest.mark.asyncio
    async def test_get_user_by_username(self, user_service, mock_repositories, sample_user):
        """測試根據用戶名獲取使用者"""
        user_repo, room_repo, message_repo = mock_repositories
    ...
    @pytest.mark.asyncio
    async def test_authenticate_user_success(self, user_service, mock_repositories, sample_user):
        """測試成功驗證使用者"""
        user_repo, room_repo, message_repo = mock_repositories
        ...
        with patch('app.services.user_service.verify_password') as mock_verify:
            with patch('app.services.user_service.create_access_token') as mock_access_token:
                with patch('app.services.user_service.create_refresh_token') as mock_refresh_token:
    ...
    @pytest.mark.asyncio
    async def test_authenticate_user_wrong_password(self, user_service, mock_repositories, sample_user):
        """測試密碼錯誤"""
        user_repo, room_repo, message_repo = mock_repositories
    ...
    @pytest.mark.asyncio
    async def test_authenticate_user_inactive(self, user_service, mock_repositories, sample_user):
        """測試不活躍使用者"""
        user_repo, room_repo, message_repo = mock_repositories
    ...
    @pytest.mark.asyncio
    async def test_update_user_success(self, user_service, mock_repositories, sample_user):
        """測試成功更新使用者"""
        user_repo, room_repo, message_repo = mock_repositories
        ...
        # 執行
        result = await user_service.update_user(sample_user.id, update_data, sample_user.id)
    ...
    @pytest.mark.asyncio
    async def test_update_user_permission_denied(self, user_service, mock_repositories, sample_user):
        """測試無權限更新他人資料"""
        user_repo, room_repo, message_repo = mock_repositories
        
        # 執行並驗證拋出異常
        with pytest.raises(ValueError) as exc_info:
            await user_service.update_user(sample_user.id, UserUpdate(), "other_user_id")
        
        assert "您只能更新自己的資料" in str(exc_info.value)
    
    @pytest.mark.asyncio
    async def test_update_user_email_exists(self, user_service, mock_repositories, sample_user):
        """測試更新信箱已存在"""
        user_repo, room_repo, message_repo = mock_repositories
        ...
        with pytest.raises(ValueError) as exc_info:
            await user_service.update_user(
                sample_user.id,
                UserUpdate(email="existing@example.com"),
                sample_user.id
            )
    ...
    @pytest.mark.asyncio
    async def test_delete_user_success(self, user_service, mock_repositories, sample_user):
        """測試成功刪除使用者"""
        user_repo, room_repo, message_repo = mock_repositories
    ...
    @pytest.mark.asyncio
    async def test_get_current_user_from_token(self, user_service, mock_repositories, sample_user):
        """測試從 token 獲取當前使用者"""
        user_repo, room_repo, message_repo = mock_repositories
    ...
    @pytest.mark.asyncio
    async def test_get_current_user_from_token_invalid(self, user_service, mock_repositories):
        """測試無效 token"""
        user_repo, room_repo, message_repo = mock_repositories
    ...
    @pytest.mark.asyncio
    async def test_get_user_activity_stats(self, user_service, mock_repositories, sample_user):
        """測試獲取使用者活動統計"""
        user_repo, room_repo, message_repo = mock_repositories
    ...
    @pytest.mark.asyncio
    async def test_change_password_success(self, user_service, mock_repositories, sample_user):
        """測試成功修改密碼"""
        user_repo, room_repo, message_repo = mock_repositories
        ...
        with patch('app.services.user_service.verify_password') as mock_verify:
            with patch('app.services.user_service.get_password_hash') as mock_hash:
                ...
                # 驗證
                assert result is True
                mock_verify.assert_called_once_with("current_password", sample_user.hashed_password)
                mock_hash.assert_called_once_with("new_password")
                user_repo.change_password.assert_called_once_with(sample_user.id, "new_hashed_password")
    
    @pytest.mark.asyncio
    async def test_change_password_wrong_current(self, user_service, mock_repositories, sample_user):
        """測試當前密碼錯誤"""
        user_repo, room_repo, message_repo = mock_repositories
    ...
    @pytest.mark.asyncio
    async def test_update_user_with_password(self, user_service, mock_repositories, sample_user):
        """測試更新使用者包含密碼"""
        user_repo, room_repo, message_repo = mock_repositories
