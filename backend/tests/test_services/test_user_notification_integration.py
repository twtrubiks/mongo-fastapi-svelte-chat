...
class TestUserNotificationIntegration:
    ...
    def user_service_with_mocks(
        self, mock_user_repository, mock_room_repository, mock_message_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_user_activity_stats_with_notification_stats(
        self,
        ...
        notification_stats = NotificationStats(
            ...
            read_count=20,
            dismissed_count=0,
            type_counts={"system": 10, "message": 15},
            recent_activity=[],
        )
        ...
        # 2. 獲取通知統計
        notification_stats_result = await notification_service.get_notification_stats(
            user_id
        )
    ...
    @pytest.mark.asyncio
    async def test_user_deactivation_with_notification_cleanup(
        self, integration_services, mock_user_repository, mock_notification_repository
    ):
        """測試用戶停用時的通知清理整合流程"""
        user_service = integration_services["user_service"]
        ...
        # 配置 NotificationService mock
        mock_notification_repository.delete_old_notifications.return_value = (
            15  # 清理了15個通知
        )
        ...
        # 2. 清理用戶相關通知
        cleanup_count = await notification_service.cleanup_old_notifications(
            days=0
        )  # 立即清理
    ...
    @pytest.mark.asyncio
    async def test_batch_user_notification_flow(
        self, integration_services, mock_user_repository, mock_notification_repository
    ):
        """測試批量用戶通知流程"""
        user_service = integration_services["user_service"]
