...
class TestRoomService:
    ...
    async def test_join_room_direct(self, room_service, mock_repositories, sample_room):
        ...
        room_repo.update_member_role.assert_called_once_with(
    ...
    async def test_delete_room_success(
        ...
        with patch("app.websocket.manager.connection_manager") as mock_conn_manager:
            ...
            # 執行
            result = await room_service.delete_room(
                sample_room.id, sample_room.owner_id
            )
    ...
    async def test_transfer_ownership(
        ...
        result = await room_service.transfer_ownership(
            sample_room.id, sample_room.owner_id, new_owner_id
        )

        # 驗證
        assert result["success"] is True
        room_repo.transfer_ownership.assert_called_once_with(
            sample_room.id, new_owner_id
        )
    ...
    async def test_broadcast_room_created(self, room_service, mock_repositories):
        ...
        mock_conn_manager.active_connections = {"user1": {}, "user2": {}, "creator": {}}
        mock_conn_manager.send_notification = AsyncMock()

        # 創建房間響應
        room_response = Mock(
            id="room123", model_dump=lambda: {"id": "room123", "name": "Test"}
        )

        # 執行
        await room_service._broadcast_room_created(
            mock_conn_manager, room_response, "creator"
        )
