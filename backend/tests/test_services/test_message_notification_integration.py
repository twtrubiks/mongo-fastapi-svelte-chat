...
class TestMessageNotificationIntegration:
    ...
    def integration_services(
        self, message_service_with_mocks, notification_service_with_mocks
    ):
        """整合測試的服務組合"""
        return {
    ...
    @pytest.mark.asyncio
    async def test_create_message_with_notification_to_room_members(
        self,
        ...
        with patch(
            ...
            message_result = await message_service.create_message(
                user_id=sender_id, message_data=message_create
            )
            ...
            batch_result = (
                await notification_service._create_batch_notifications_business_logic(
                    user_ids=notification_user_ids,
                    title="來自 sender 的新訊息",
                    content="在 測試房間 中：Hello everyone!",
    ...
    @pytest.mark.asyncio
    async def test_reply_message_with_targeted_notification(
        self,
        integration_services,
        mock_message_repository,
        mock_room_repository,
        ...
        with patch(
            ...
            reply_result = await message_service.create_message(
                user_id=reply_sender_id, message_data=reply_create
            )
    ...
    @pytest.mark.asyncio
    async def test_private_message_notification(
        self,
        integration_services,
        mock_message_repository,
        mock_room_repository,
        ...
        with patch(
            ...
            private_create = MessageCreate(
                room_id=room_id, content="私人消息內容", message_type=MessageType.TEXT
            )

            message_result = await message_service.create_message(
                user_id=sender_id, message_data=private_create
            )
    ...
    @pytest.mark.asyncio
    async def test_message_deletion_with_notification_cleanup(
        self,
        integration_services,
        mock_message_repository,
        mock_notification_repository,
        ...
        """測試消息刪除時的通知清理整合流程"""
        message_service = integration_services["message_service"]
        _notification_service = integration_services[
        ...
        if hasattr(mock_notification_repository, "delete_by_message_id"):
            cleanup_count = await mock_notification_repository.delete_by_message_id(
                message_id
            )
    ...
    @pytest.mark.asyncio
    async def test_message_search_with_notification_context(
        self,
        integration_services,
        mock_message_repository,
        mock_notification_repository,
        ...
        search_result = await message_service.search_messages(
            room_id=room_id, search_query=search_query
        )
        ...
        mock_message_repository.search_messages.assert_called_once_with(
            room_id, search_query
        )
        mock_notification_repository.create.assert_called_once()
        notification_service.send_real_time_notification.assert_called_once()

    @pytest.mark.asyncio
    async def test_error_handling_in_message_notification_integration(
        self,
        integration_services,
        mock_message_repository,
        mock_room_repository,
        ...
        with patch(
            ...
            # 1. 創建消息應該成功
            message_create = MessageCreate(
                room_id=room_id, content="消息內容", message_type=MessageType.TEXT
            )
