...
class TestRoomMessageIntegration:
    ...
    async def test_create_room_and_send_welcome_message(
        ...
        # 執行整合流程
        # 1. 創建房間（connection_manager 已透過建構子注入 mock）
        ...
        with patch(
            "app.services.message_service.UserRepository"
    ...
    async def test_join_room_and_send_join_message(
        ...
        mock_room_repository.add_member.return_value = True
        mock_room_repository.is_member.return_value = True
        ...
        # 驗證服務調用
        mock_room_repository.add_member.assert_called_once_with(
            room_id, new_user_id, MemberRole.MEMBER
        )
        mock_message_repository.create.assert_called_once()
    ...
    async def test_delete_room_with_message_cleanup(
        ...
        # 執行整合流程
        # 1. 刪除房間
        await room_service.delete_room(room_id, owner_id)
