...
class TestFastAPIIntegration:
    ...
    async def test_create_dependency_factory(self):
        ...
        # 創建依賴工廠
        with patch(
            "app.core.fastapi_integration.get_container", return_value=mock_container
        ):
    ...
    async def test_setup_dependency_injection(self):
        ...
        with patch(
            ...
            with patch(
                "app.core.fastapi_integration.configure_services"
            ) as mock_configure:
                # 執行設置
    ...
    async def test_setup_dependency_injection_error(self):
        ...
        # Mock 錯誤
        with patch(
            "app.core.fastapi_integration.get_container",
    ...
    async def test_request_scope_middleware(self):
        ...
        mock_request.app.state.di_container.clear_scope.assert_called_with(
            "test-scope-id"
        )
    ...
    async def test_get_health_check_info_success(self):
        ...
        with patch(
            "app.core.service_registration.health_check_services",
            return_value=mock_health_info,
