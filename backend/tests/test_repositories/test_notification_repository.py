...
from app.models.notification import (
    NotificationInDB, NotificationCreate, NotificationUpdate,
...
class TestNotificationRepository:
    ...
    async def test_get_by_user(self, db_manager_sync):
        ...
        notifications = [
            NotificationCreate(
                ...
                title=f"通知 {i}",
                content=f"內容 {i}",
                type=NotificationType.MESSAGE if i % 2 == 0 else NotificationType.SYSTEM
            )
            for i in range(5)
        ]
        ...
        assert all(notif.type == NotificationType.MESSAGE for notif in message_notifications)
    ...
    async def test_get_by_user_with_status_filter(self, db_manager_sync):
        ...
        created_notification2 = await repo.create_from_data(notification2)
        ...
        assert all(notif.status == NotificationStatus.UNREAD for notif in unread_notifications)
        ...
        assert all(notif.status == NotificationStatus.READ for notif in read_notifications)
