...
class TestCoreBusinessSimple:
    ...
    async def test_user_registration_flow(self):
        ...
        # 設置 Mock 行為
        mock_user_repo.get_by_username.return_value = None  # 用戶不存在
        mock_user_repo.get_by_email.return_value = None  # 郵箱不存在
        ...
        # 檢查用戶是否存在
        existing_user = await mock_user_repo.get_by_username(username)
        existing_email = await mock_user_repo.get_by_email(email)
    ...
    async def test_join_room_validation(self):
        ...
        mock_room_repo.get_by_id.return_value = room_data

        # 測試加入房間的驗證
        room = await mock_room_repo.get_by_id(room_id)
