...
class TestWebSocketHandlersAdditional:
    ...
    async def test_handle_websocket_connection_success(
        self, mock_websocket, mock_user_info
    ):
        """測試 WebSocket 連線成功建立"""
        with (
            patch("app.websocket.handlers.websocket_auth_middleware") as mock_auth,
    ...
    async def test_handle_message_chat_message(self, mock_user_info):
        """測試處理聊天訊息"""
        message_data = {"type": "chat_message", "content": "Hello world"}
        ...
        with patch("app.websocket.handlers.handle_chat_message") as mock_handle:
            ...
            mock_handle.assert_called_once_with(
                "user123", "room123", message_data, mock_user_info
            )
    ...
    async def test_handle_message_notification_read(self, mock_user_info):
        """測試處理通知已讀"""
        message_data = {"type": "notification_read", "notification_id": "notif123"}
    ...
    async def test_handle_chat_message_empty_content(self, mock_user_info):
        """測試處理空內容聊天訊息"""
        message_data = {"content": "", "temp_id": "temp123"}
    ...
    @pytest.mark.asyncio
    async def test_handle_chat_message_image_type_long_content(
        self, mock_user_info, mock_message_service
    ):
        """測試處理圖片類型長內容（應該允許）"""
        long_content = "x" * 1500
        message_data = {
        ...
        mock_message_service.create_message.return_value = mock_created_message
    ...
    @pytest.mark.asyncio
    async def test_handle_chat_message_success_with_notifications(
        self,
        mock_user_info,
        ...
        mock_room_service.get_room_members.return_value = [mock_member1, mock_member2]
        ...
        with (
            ...
            patch(
                "app.websocket.handlers.create_room_service",
                return_value=mock_room_service,
            ),
            patch(
                "app.websocket.handlers.create_notification_service",
    ...
    async def test_handle_chat_message_service_error(self, mock_user_info):
        """測試聊天訊息服務錯誤"""
        message_data = {"content": "Hello world", "temp_id": "temp123"}
    ...
    @pytest.mark.asyncio
    async def test_handle_notification_read_single_notification(self, mock_user_info):
        """測試處理單個通知已讀"""
        message_data = {"notification_id": "notif123", "read_type": "notification"}
        ...
        with (
            ...
            response_calls = [
                call
                for call in mock_cm.send_personal_message.call_args_list
            ...
            # 驗證狀態廣播
            mock_cm.send_event.assert_called_once()
            event_data = mock_cm.send_event.call_args[0][1]
            assert event_data["type"] == "notification_status_changed"
            assert event_data["data"]["read_type"] == "notification"
            assert event_data["data"]["id"] == "notif123"
            assert event_data["data"]["status"] == "read"

    @pytest.mark.asyncio
    async def test_handle_notification_read_room_notifications(self, mock_user_info):
        """測試處理房間通知已讀"""
        message_data = {"read_type": "room", "target_room_id": "room456"}

        mock_notification_service = Mock()
        mock_notification_service.mark_room_notifications_as_read = AsyncMock(
            return_value=5
        )
        ...
        with (
            ...
            # 驗證標記房間通知已讀
            mock_notification_service.mark_room_notifications_as_read.assert_called_once_with(
                "user123", "room456"
            )
            ...
            event_data = mock_cm.send_event.call_args[0][1]
            assert event_data["type"] == "notification_status_changed"
            assert event_data["data"]["read_type"] == "room"
            assert event_data["data"]["id"] == "room456"
            assert event_data["data"]["status"] == "read"

    @pytest.mark.asyncio
    async def test_handle_notification_read_all_notifications(self, mock_user_info):
        """測試處理所有通知已讀"""
        ...
        with (
            ...
            # 驗證標記所有通知已讀
            mock_notification_service.mark_all_as_read.assert_called_once_with(
                "user123"
            )
            ...
            event_data = mock_cm.send_event.call_args[0][1]
            assert event_data["type"] == "notification_status_changed"
            assert event_data["data"]["read_type"] == "all"
            assert event_data["data"]["status"] == "read"

    @pytest.mark.asyncio
    async def test_handle_notification_read_service_error(self, mock_user_info):
        """測試處理通知已讀服務錯誤"""
        message_data = {"notification_id": "notif123", "read_type": "notification"}
        ...
        with (
            ...
            # 驗證發送失敗響應（success=False）
            error_calls = [
                call
                for call in mock_cm.send_personal_message.call_args_list
                if call[0][2].get("type") == "read_status_response"
                and not call[0][2].get("success")
            ]
    ...
    async def test_send_recent_messages_success(self):
        ...
        mock_message_service = Mock()
        mock_message_service.get_room_messages = AsyncMock(return_value=[mock_message])
    ...
    async def test_send_recent_messages_service_error(self):
        """測試發送最近訊息服務錯誤"""
        mock_message_service = Mock()
        mock_message_service.get_room_messages = AsyncMock(
            side_effect=Exception("Service error")
        )
