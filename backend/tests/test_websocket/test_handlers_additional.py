...
class TestWebSocketHandlersAdditional:
    ...
    def mock_websocket(self):
        """Mock WebSocket"""
        websocket = Mock()
        websocket.accept = AsyncMock()  # 添加 accept mock
    ...
    @pytest.fixture
    def mock_container(
        self, mock_message_service, mock_room_service, mock_notification_service
    ):
        """Mock Container"""
        container = Mock()
    ...
    async def test_handle_websocket_connection_success(
        self, mock_websocket, mock_user_info, mock_container
    ):
        """測試 WebSocket 連線成功建立"""
        with (
    ...
    async def test_handle_message_chat_message(self, mock_user_info):
        ...
        with patch("app.websocket.handlers.handle_chat_message") as mock_handle:
            ...
            await handle_message(
                "user123", "room123", message_data, mock_user_info, "scope123"
            )
    ...
    async def test_handle_chat_message_image_type_long_content(self, mock_user_info):
        ...
        with (
            ...
            mock_message_service.create_message = AsyncMock(
                return_value=mock_created_message
            )
    ...
    async def test_handle_chat_message_success_with_notifications(
        self, mock_user_info, mock_container
    ):
        """測試成功處理聊天訊息並發送通知"""
        message_data = {
            "content": "Hello world",
    ...
    @pytest.mark.asyncio
    async def test_handle_notification_read_single_notification(
        self, mock_user_info, mock_container
    ):
        """測試處理單個通知已讀"""
    ...
    @pytest.mark.asyncio
    async def test_handle_notification_read_room_notifications(
        self, mock_user_info, mock_container
    ):
        """測試處理房間通知已讀"""
    ...
    @pytest.mark.asyncio
    async def test_handle_notification_read_all_notifications(
        self, mock_user_info, mock_container
    ):
        """測試處理所有通知已讀"""
    ...
    @pytest.mark.asyncio
    async def test_handle_notification_read_service_error(
        self, mock_user_info, mock_container
    ):
        """測試處理通知已讀服務錯誤"""
        message_data = {"notification_id": "notif123", "read_type": "notification"}
