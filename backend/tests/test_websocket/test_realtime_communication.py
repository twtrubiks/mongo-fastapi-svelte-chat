...
class TestRealtimeCommunication:
    ...
    def test_message_broadcasting(self):
        ...
        class MessageBroadcaster:
            ...
            def broadcast_message(self, room_id: str, message: dict, exclude_user: str = None):
                # 記錄指標
            ...
            def broadcast_typing_indicator(self, room_id: str, user_id: str, is_typing: bool):
                """廣播打字指示器"""
                message = {
                    "type": "typing",
                    "user_id": user_id,
        ...
        broadcaster.connection_manager.get_connections_for_room.return_value = mock_connections
    ...
    def test_message_acknowledgment(self):
        ...
        class MessageAcknowledgment:
            ...
            def get_unacknowledged_count(self, connection=None):
                ...
                if connection:
                    count = sum(
                        1 for info in self.pending_messages.values()
    ...
    def test_rate_limiting_per_connection(self):
        ...
        class ConnectionRateLimiter:
            ...
            def get_remaining_quota(self, connection_id: str, action: str):
                ...
                if bucket_key in self.buckets:
                    now = datetime.now(UTC).timestamp()
                    cutoff = now - limit["window"]
                    current_count = sum(1 for t in self.buckets[bucket_key] if t > cutoff)
        ...
        # 測試達到限制
        for i in range(9):  # 已經有1個，再加9個達到10個
    ...
    def test_message_history_sync(self):
        ...
        class MessageHistorySync:
            ...
            def sync_history(self, user_id: str, room_id: str, last_sync_time=None):
                ...
                # 獲取訊息
                messages = list(self.db.messages.find(query).sort("created_at", -1).limit(self.page_size))
            ...
            def get_undelivered_messages(self, user_id: str):
                ...
                undelivered = [
                    ...
                    if str(msg["_id"]) not in delivered_ids
                ]
        ...
        # 設置 Mock
        mock_messages = [
            {"_id": "msg1", "room_id": "room1", "content": "Hello", "created_at": "2024-01-01T10:00:00"},
            {"_id": "msg2", "room_id": "room1", "content": "Hi", "created_at": "2024-01-01T10:01:00"}
        ]
        
        mock_db.messages.find.return_value.sort.return_value.limit.return_value = mock_messages
        mock_db.read_status.find.return_value = [{"message_id": "msg1", "user_id": "user1"}]
        mock_db.delivery_status.insert_many.return_value = Mock(inserted_ids=["d1", "d2"])
    ...
    def test_connection_recovery(self):
        ...
        recovery.session_store.get.return_value = json.dumps({
            ...
            "disconnected_at": (datetime.now(UTC) - timedelta(seconds=30)).isoformat()
        })
    ...
    def test_room_state_management(self):
        ...
        # 測試用戶狀態
        manager.update_user_state("user1", "room1", {
            "status": "away",
            "unread_count": 5,
