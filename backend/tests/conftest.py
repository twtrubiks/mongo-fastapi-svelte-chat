...
def db_manager_sync(event_loop):
    ...
    async def _get_db_manager():
        ...
        await manager.connect(
            TestConfig.TEST_MONGODB_URL, TestConfig.TEST_DATABASE_NAME
        )
        await manager.clean_database()
...
def test_app_setup(event_loop):
    ...
    event_loop.run_until_complete(
        init_database(TestConfig.TEST_MONGODB_URL, TestConfig.TEST_DATABASE_NAME)
    )
...
def user_service_with_mocks(
    mock_user_repository, mock_room_repository, mock_message_repository
):
