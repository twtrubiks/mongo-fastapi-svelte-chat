...
class TestDataValidation:
    ...
    def test_user_model_validation(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            UserModel(
                username="jo", email="test@example.com", password="password123"  # 太短
            )
        ...
        with pytest.raises(ValueError) as exc_info:
            UserModel(
                username="user123", email="user@example.com", password="pass"  # 太短
            )
    ...
    def test_message_model_validation(self):
        ...
        valid_message = MessageModel(
            room_id="room123", sender_id="user123", content="Hello, world!"
        )
    ...
    def test_room_model_validation(self):
        ...
        # 測試無效的房間名稱
        with pytest.raises(ValueError) as exc_info:
            RoomModel(name="12345", owner_id="user123")
    ...
    def test_api_request_validation(self):
        ...
        # 測試搜索參數
        valid_search = SearchParams(query="python programming", sort_by="created_at")
    ...
    def test_business_logic_validation(self):
        ...
        class BusinessValidator:
            ...
            def validate_message_sending(
                self, user_id: str, room_id: str, message: dict
            ):
                """驗證訊息發送"""
                errors = []
        ...
        valid, errors = validator.validate_room_creation(
            "user123", {"name": "New Room", "room_type": "public"}
        )
        ...
        valid, errors = validator.validate_room_creation(
            "user123", {"name": "Existing Room", "room_type": "public"}
        )
    ...
    def test_input_normalization(self):
        ...
        # 測試電子郵件正規化
        assert (
            InputNormalizer.normalize_email("John.Doe+test@GMAIL.com")
            == "johndoe@gmail.com"
        )
        assert (
            InputNormalizer.normalize_email("  USER@EXAMPLE.COM  ")
            == "user@example.com"
        )
        ...
        # 測試空白正規化
        assert (
            InputNormalizer.normalize_whitespace("  multiple   spaces  ")
            == "multiple spaces"
        )
