...
class TestCoreWorkflows:
    ...
    def test_user_registration_workflow(self):
        ...
        workflow = RegistrationWorkflow(
            mock_user_service, mock_email_service, mock_notification_service
        )
    ...
    def test_room_join_workflow(self):
        ...
        class RoomJoinWorkflow:
            ...
            def join_room(self, user_id, room_id):
                ...
                # 3. 發送系統訊息
                self.message.create_system_message(room_id, "User joined the room")
                ...
                for member in members:
                    if member != user_id:
                        self.notification.notify_user(
                            member, f"New member joined {room['name']}"
                        )

                return {"joined": True, "room": room}

        # 設置 Mock 返回值
        mock_room_service.get_room.return_value = {"id": "room1", "name": "Test Room"}
        ...
        workflow = RoomJoinWorkflow(
            mock_room_service, mock_notification_service, mock_message_service
        )
    ...
    def test_file_sharing_workflow(self):
        ...
        class FileSharingWorkflow:
            ...
            def share_file(self, user_id, room_id, file_data):
                ...
                message = self.message.create_file_message(
                    user_id, room_id, file_url, file_data["name"]
                )

                return {"file_url": file_url, "message": message}
        ...
        workflow = FileSharingWorkflow(
            mock_file_service, mock_message_service, mock_storage_service
        )

        result = workflow.share_file(
            "user1", "room1", {"name": "test.jpg", "data": b"fake"}
        )
    ...
    def test_notification_delivery_workflow(self):
        ...
        class NotificationDeliveryWorkflow:
            ...
            def deliver_notification(self, user_id, notification):
                ...
                return {"delivered_via": delivered, "success": len(delivered) > 0}
        ...
        result = workflow.deliver_notification(
            "user1", {"title": "New Message", "body": "You have a new message"}
        )
