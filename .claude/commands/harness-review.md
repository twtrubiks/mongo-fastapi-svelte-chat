...
|-----------|-------|----------|------|----------|
| {event_type} | {欄位名} | {service}.py:{行號} | 前端缺失 | major |

### any 滲透
| 風險層級 | 數量 | 代表案例 |
|----------|------|----------|
| 高（WS handler） | N 處 | `frontend/src/lib/websocket/` 某 handler — 參數 `data: any` |
| 中（BFF/API） | N 處 | `frontend/src/lib/bff-*.ts` — `BFFResponse<any>` / `catch (error: any)` |
| 低（元件/工具） | N 處 | `frontend/src/routes/**/+page.svelte` — 元件 ref `: any` |
```
