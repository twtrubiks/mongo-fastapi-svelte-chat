...
# --- 規則 1：禁止寫入 .env 檔案（範本檔 .env.example/.sample/.template 放行，入版控且不含 secret）---
if echo "$FILE_PATH" | grep -qE '\.env$|\.env\.' && ! echo "$FILE_PATH" | grep -qE '\.env\.(example|sample|template)$' ; then
